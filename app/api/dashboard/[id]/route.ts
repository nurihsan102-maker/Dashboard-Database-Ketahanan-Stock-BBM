...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: upload, error: e0 }, { data: ketahanan, error: e1 }, { data: status, error: e2 }, { data: coverage, error: e3 }] =
    await Promise.all([
  ...
  return NextResponse.json({ upload, ketahanan, status, coverage });
}
